...
public class MiniPID {
    ...
    public double getOutput(double actual, double setpoint, double timeAdj){
        ...
        // See if we passed through the setpoint (avoid /0)
        if(ReduceErrorAtSetpoint != 1.0) {
            if (error != 0.0 ) {
                // A sign change here means we passed through the setpoint
                if((setpoint-lastActual) / error <= 0.0) {
                    errorSum = errorSum * ReduceErrorAtSetpoint;
                }
            } else {
                errorSum = errorSum * ReduceErrorAtSetpoint;
            }
        ...
        if(minOutput!=maxOutput && !bounded(output, minOutput,maxOutput) ){
            // Don't squash I here, just don't build anymore
            // errorSum=error*timeAdj;

            // Alternatively, reset the error sum to a sane level
        ...
        if(outputRampRate!=0){
            output=constrain(output, lastOutput-(outputRampRate*timeAdj),lastOutput+(outputRampRate*timeAdj));
        }
    ...
    private void checkSigns(){
        if(reversed){  // all values should be below zero
            ...
            if(I>0) I*=-1;
            if(D>0) D*=-1;
            //if(F>0) F*=-1;  // tas 11/24/23 -- F doesn't need to be the same sign
        }
